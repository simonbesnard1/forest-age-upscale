...
FIA_data['age'] = np.round(FIA_data['age'])
...
train_data['age_class'] = np.round(train_data['age'], -1)
age_counts = train_data['age_class'].value_counts()


import numpy as np
import pandas as pd

# Define the desired number of points per class

# Calculate the current number of points per class
current_points_per_class = train_data['age_class'].value_counts().sort_index()
...
for age_class, current_points in current_points_per_class.items():
    ...
    if age_class==300:
        desired_points_per_class = 7000
    else: 
        desired_points_per_class = 3000
    ...
    if required_samples > 0:
        ...
        oversampled_data = pd.concat([train_data[train_data['age_class'] == age_class], oversampled_data])
...
# Export to csv
balanced_data.to_csv('/home/simon/Documents/science/GFZ/projects/forest-age-upscale/data/training_data/training_data_ageMap_OG300_v3.csv')
