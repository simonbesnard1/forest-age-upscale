...
def determine_next_age_class(current_age_class):
    # Define the order of age classes
...
for member_ in np.arange(20):
    ...
    age_2020 = xr.open_zarr('/home/simon/hpc_group/scratch/besnard/upscaling/Age_upscale_100m/XGBoost/version-1.0/AgeClass_1deg').sel(time = '2020-01-01').sel(members= member_).transpose("latitude", "longitude", "age_class").forest_age
    ...
    counts_2010 = np.stack(total_area_per_age_class_scenario1.values())
    ...
    BiomassDiffPartition_1deg =  xr.open_zarr('/home/simon/hpc_group/scratch/besnard/upscaling/Age_upscale_100m/XGBoost/version-1.0/BiomassPartition_1deg').sel(members = member_)
    ...
    OG_stand_replaced_total = OG_stand_replaced * pixel_area * forest_fraction  * OG_stand_replaced_class.values
    ...
    OG_aging_total = OG_aging * pixel_area * forest_fraction * OG_aging_class.values 
    ...
    total_budget_OG_stand_replaced = np.nansum(OG_stand_replaced_total)
    total_budget_OG_stand_replaced = np.nansum(total_budget_OG_stand_replaced) * 1e-13
    ...
    total_budget_OG_aging = np.nansum(OG_aging_total)
    total_budget_OG_aging = np.nansum(total_budget_OG_aging) * 1e-13
    
    total_young = total_budget_Young_stand_replaced +  total_budget_Intermediate_stand_replaced + total_budget_Mature_stand_replaced + total_budget_Young_aging  + total_budget_OG_stand_replaced
    
    total_budget_BAU = total_young + total_budget_OG_aging +  total_budget_Intermediate_aging + total_budget_Mature_aging
    ...
    total_budget_OG_scenario1 = (total_budget_OG_aging * summarized_areas_scenario1['OG forests \n (>200 years)']) /  summarized_areas_2020['OG forests \n (>200 years)']
    
    total_budget_scenario1 = total_budget_Young_scenario1 + total_budget_Intermediate_scenario1 +  total_budget_Mature_scenario1 + total_budget_OG_scenario1
    ...
    AgePartition_total_scenario1.append(pd.DataFrame({'member': [member_], 'Young stand-replaced':[total_budget_Young_scenario1], 'Maturing stand-replaced':[total_budget_Intermediate_scenario1], 
                                                 'Mature stand-replaced':[total_budget_Mature_scenario1], 'OG stand-replaced':[total_budget_OG_scenario1], 'All stand-replaced':[total_budget_scenario1]}))
    ...
    AgePartition_total_BAU.append(pd.DataFrame({'member': [member_], 'Young stand-replaced':[total_young], 'Maturing stand-replaced':[total_budget_Intermediate_aging], 
                                           'Mature stand-replaced':[total_budget_Mature_aging],'OG stand-replaced':[total_budget_OG_aging], 'All stand-replaced':[total_budget_BAU]}))
...
fig, ax = plt.subplots(1, 2, figsize=(17.5, 7), gridspec_kw={'wspace': 0, 'hspace': 0.0}, constrained_layout=True)
...
ax[1].legend(frameon=False, fontsize=14, bbox_to_anchor=(0.6,-0.2))
ax[1].text(0.05, 1.05, 'b', transform=ax[1].transAxes,
            fontsize=16, fontweight='bold', va='top')
ax[1].set_title('Forest age class distribution in 2050', fontsize=14, fontweight='bold')
...
ax[0].text(0.05, 1.05, 'a', transform=ax[0].transAxes,
            fontsize=16, fontweight='bold', va='top')
ax[0].set_title('Total carbon stock in 2050', fontsize=14, fontweight='bold')
plt.savefig('/home/simon/Documents/science/research_paper/global_age_Cdyn/figs/fig4.png', dpi=300)
