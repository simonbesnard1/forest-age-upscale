...
for i, region_ in enumerate(transcom_mask.keys()):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(age_classes, rotation=90, fontsize=14)
    ...
    ax.set_title(class_name, fontweight='bold', fontsize=16)
    ax.text(0.05, 1.05, f'({chr(97 + i)})', transform=ax.transAxes,
            fontsize=16, fontweight='bold', va='top')
fig.delaxes(axes[-1])
plt.savefig(os.path.join(plot_dir,'figExt1.pdf'), dpi=300)
