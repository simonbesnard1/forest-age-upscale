...
for j, region_ in enumerate(transcom_mask.keys()):
    ...
    ax.set_xticklabels(list(AgePartition.keys()), rotation=90, size=16)
    
    # Rest of your plot settings
    ax.set_ylabel('Annual growth rate [percent]', size=16)
    ax.spines['top'].set_visible(False)
    ax.spines['right']. set_visible(False)
    ax.tick_params(labelsize=12)
    ax.set_title(class_name, fontweight='bold', fontsize=18)
    ax.text(0.05, 1.05, f'({chr(97 + j)})', transform=ax.transAxes,
