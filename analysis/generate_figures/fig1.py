...
average_age = xr.open_zarr(os.path.join(data_dir,'ForestAge_1deg')).forest_age.mean(dim = 'time').median(dim =  'members').where(forest_fraction>.2)
age_difference = xr.open_zarr(os.path.join(data_dir,'AgeDiff_1deg')).age_difference.median(dim =  'members').where(forest_fraction>.2)
...
for member_ in np.arange(20):
    ...
    for age_class in age_class_2010.age_class.values:
        ...
        total_area_per_age_class_2010[age_class] = (age_class_2010.sel(age_class = age_class).forest_age.where(forest_fraction>.2) * pixel_area * forest_fraction).sum(dim=['longitude', 'latitude']).values / 10**7
        total_area_per_age_class_2020[age_class] = (age_class_2020.sel(age_class = age_class).forest_age.where(forest_fraction>.2) * pixel_area * forest_fraction).sum(dim=['longitude', 'latitude']).values / 10**7
...
fig.set_constrained_layout_pads(w_pad=0, h_pad=0.1, hspace=0, wspace=0.1)
