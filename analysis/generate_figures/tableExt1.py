...
for member_ in np.arange(20):
    age_class_2010 = xr.open_zarr(os.path.join(data_dir,'AgeClass_1deg')).sel(time = '2010-01-01', members=member_).where(forest_fraction >0.2) 
    age_class_2020 = xr.open_zarr(os.path.join(data_dir,'AgeClass_1deg')).sel(time = '2020-01-01', members=member_).where(forest_fraction >0.2)
    forest_fraction = xr.open_zarr(os.path.join(data_dir,'ForestFraction_1deg')).forest_fraction
