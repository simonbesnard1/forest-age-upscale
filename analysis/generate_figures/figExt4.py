...
Intermediate_aging_class = AgeDiffPartition_fraction_1deg.aging_forest_class_partition.where(forest_fraction >0.2).sel(age_class = ['20-80']).sum(dim='age_class')
Intermediate_aging_class = Intermediate_aging_class.where(Intermediate_aging_class >0)

Mature_aging_class =  AgeDiffPartition_fraction_1deg.aging_forest_class_partition.where(forest_fraction >0.2).sel(age_class = ['80-200']).sum(dim='age_class')
...
ax[0,0].coastlines()
ax[0,0].gridlines()
ax[0,0].set_title('Fraction of undisturbed ageing young forests \n (1-20 years)')
...
ax[0,1].coastlines()
ax[0,1].gridlines()
ax[0,1].set_title('Fraction of undisturbed ageing maturing forests \n (21-80 years)')
...
ax[1,0].coastlines()
ax[1,0].gridlines()
ax[1,0].set_title('Fraction of undisturbed ageing mature forests \n (81-200 years)')
...
ax[1,1].coastlines()
ax[1,1].gridlines()
ax[1,1].set_title('Fraction of undisturbed ageing old-growth forests \n ($>$200 years)')
ax[1,1].text(0.01, 1.15, '(d)', transform=ax[1,1].transAxes,
            fontsize=16, fontweight='bold', va='top')
plt.savefig(os.path.join(plot_dir,'figExt4.pdf'), dpi=300)
