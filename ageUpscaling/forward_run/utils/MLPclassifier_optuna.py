...
class model_opti:
    ...
    def tune_(self):
        ...
        if not os.path.exists(self.tune_dir + '/save_model/run' + str(self.iter_)):
            os.makedirs(self.tune_dir + '/save_model/run' + str(self.iter_))
        
        study = optuna.create_study(study_name = 'Age_run' + str(self.iter_), 
                                    storage='sqlite:///' + self.tune_dir + '/save_model/run' + str(self.iter_) + '/hp_trial.db',
        ...
        with open(self.tune_dir + '/save_model/run' + str(self.iter_) + '/model_trial' + "{}.pickle".format(study.best_trial.number), "rb") as fin:
    ...
    def _objective(self, trial: optuna.Trial, x_train, x_valid, y_train, y_valid, iter_, tune_dir):
        ...
        with open(tune_dir + '/save_model/run' + str(iter_) + '/model_trial' + "{}.pickle".format(trial.number), "wb") as fout:
