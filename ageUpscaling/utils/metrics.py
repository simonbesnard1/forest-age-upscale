...
def mef_gufunc(obs, mod):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)

        valid_values = np.isfinite(obs) & np.isfinite(mod)

        obs[~valid_values] = np.nan
        mod[~valid_values] = np.nan

        sse = np.nansum(np.power(obs-mod, 2), axis=-1)
        sso = np.nansum(
            np.power(obs-np.nanmean(obs, axis=-1, keepdims=True), 2), axis=-1)
